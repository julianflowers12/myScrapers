...
```{r}
if(!require(myScrapers))devtools::install_github("julianflowers12/myScrapers")
library(myScrapers)
```
...
topics <- abstract_topics(k = 20, x = np)
topics$model
...
topic <- myScrapers::abstract_topic_viz(x = np, m = topics$model, scores = topics$scores, n = 20)
...
figures <- map(1:20, ~(abstract_topic_viz(x = np, m = topics$model, scores = topics$scores, n = .x)))
